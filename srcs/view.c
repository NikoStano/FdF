...
/*   Updated: 2025/08/12 15:40:26 by nistanoj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_zoom(t_fdf *fdf, double factor, int pivot_x, int pivot_y)
{
	...
	new_scale = clampi((int)round(old_scale * factor), MIN_SCALE, \
		MAX_SCALE); 
    if (new_scale == old_scale)
        return ;
    k = (double)new_scale / (double)old_scale;

    fdf->view->offset_x = pivot_x + (int)round((fdf->view->offset_x - \
		pivot_x) * k);
    fdf->view->offset_y = pivot_y + (int)round((fdf->view->offset_y - \
		pivot_y) * k);
