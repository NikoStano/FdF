/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   hooks.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: nistanoj <nistanoj@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/21 12:34:50 by nistanoj          #+#    #+#             */
/*   Updated: 2025/10/27 08:19:15 by nistanoj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fdf.h"

static int	handle_movement(int key, t_app *a)
{
	if (key == XK_Left || key == XK_a)
	{
		a->keys.left = 1;
		a->view.off_x -= STEP;
	}
	else if (key == XK_Right || key == XK_d)
	{
		a->keys.right = 1;
		a->view.off_x += STEP;
	}
	else if (key == XK_Up || key == XK_w)
	{
		a->keys.up = 1;
		a->view.off_y -= STEP;
	}
	else if (key == XK_Down || key == XK_s)
	{
		a->keys.down = 1;
		a->view.off_y += STEP;
	}
	else
		return (0);
	return (1);
}

static int	handle_view_part1(int key, t_app *a)
{
	if (key == XK_r)
		view_reset(&a->view);
	else if (key == XK_p)
	{
		a->view.projection = (a->view.projection + 1) % 3;
		print_projection(a->view.projection);
	}
	else if (key == XK_c)
		a->view.color_mode = (a->view.color_mode + 1) % 2;
	else if (key == XK_space)
		a->view.auto_rotate = !a->view.auto_rotate;
	else
		return (0);
	return (1);
}

static int	handle_view_part2(int key, t_app *a)
{
	if (key == XK_Shift_L && a->view.z_scale <= 15)
	{
		a->keys.z_plus = 1;
		a->view.z_scale += 1;
	}
	else if (key == XK_Control_L && a->view.z_scale > 1)
	{
		a->keys.z_minus = 1;
		a->view.z_scale -= 1;
	}
	else if (key == XK_plus || key == XK_KP_Add)
	{
		a->keys.zoom_in = 1;
		a->view.scale = clampi(a->view.scale + 5, MIN_SCALE, MAX_SCALE);
	}
	else if (key == XK_minus || key == XK_KP_Subtract)
	{
		a->keys.zoom_out = 1;
		a->view.scale = clampi(a->view.scale - 5, MIN_SCALE, MAX_SCALE);
	}
	else
		return (0);
	return (1);
}

static int	handle_view(int key, t_app *a)
{
	int	r;

	r = handle_view_part1(key, a);
	if (r)
		return (r);
	return (handle_view_part2(key, a));
}

int	on_key(int key, t_app *a)
{
	int	handled;

	if (key == XK_Escape)
		return (on_destroy(a));
	handled = handle_movement(key, a);
	handled = handled || handle_view(key, a);
	handled = handled || handle_rotation(key, a);
	if (handled)
	{
		render(a);
		return (0);
	}
	return (0);
}
